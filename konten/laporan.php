...
<div class="modal fade" id="modalPerProduk" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Pilih Siswa dan Periode Laporan</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="pdf/output/laporan_per_produk.php" method="get" target="_blank">
            <label for="ProdukID">Produk</label>
            <Select name="ProdukID" class="form-control" required>
              <option value="">-- Pilih Produk --</option>
              <?php
                  $sql_penjualan_produk="SELECT * FROM produk ORDER BY NamaProduk";
                  $query=mysqli_query($koneksi,$sql_penjualan_produk);
